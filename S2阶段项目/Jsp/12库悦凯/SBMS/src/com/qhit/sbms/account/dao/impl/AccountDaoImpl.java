package com.qhit.sbms.account.dao.impl;
...
import com.qhit.sbms.account.bean.AccountDetail;
import com.qhit.sbms.account.dao.AccountDao;
...
public class AccountDaoImpl implements AccountDao {
	...
	public PageBean getPageBeanByParam(List<String> wheres, List<String> values) {
		...
		String sql = " select a.accountId,g.goodsName,g.goodsNum, "
					 ...
					 +" from tb_account a left join "
					 +" tb_goods g on a.goodsId = g.goodsId  left join "
					 +" tb_provider p on p.providerId = a.providerId ";
	...
	public int upDateAccountById(int accountId, int providerId, int isPayed) {
		con = DBManager.getConnection();
		String sql = "update tb_account set provider = ? , isPayed = ? where accountId = ? ";
		try {
			ps = con.prepareStatement(sql);
			ps.setInt(1, providerId);
			ps.setInt(2, isPayed);
